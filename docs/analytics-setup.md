...
**Date filtering uses Eastern Time, per Postmark's documented format.** `fromdate`/`todate` (bounces and deliveries) are converted to naive Eastern Time timestamps (`YYYY-MM-DDT12:00:00`, no UTC offset), matching Postmark's documented format exactly, using the IANA `America/New_York` zone so daylight saving is handled correctly year-round. `options.begin`/`options.end` themselves are still plain JavaScript `Date`s (or ISO strings) in whatever zone you construct them — only the outgoing API request is Eastern-Time-formatted.
...
**Event ordering and Ghost's polling cursor.** Ghost's analytics batch processor advances its cursor to the newest timestamp in each batch it processes, then uses that as the start of the next poll's window. Postmark's list endpoints are conventionally newest-first (undocumented; unverified against the live API). Dispatching events in whatever order the API returned them could advance the cursor past older events the run never looked at, permanently skipping them. To avoid this, each event category (bounces, opens, deliveries) collects every page it fetches and sorts oldest-first before calling `batchHandler`, so a truncated batch never causes the cursor to jump ahead of unprocessed events *within what was fetched*. This is fully correct whenever `options.maxEvents` is unbounded (the common case) or all available pages get fetched. If `maxEvents` is small enough to stop pagination before reaching the true oldest page — a scenario Postmark's list APIs offer no way to avoid without fetching everything, since there's no way to request "oldest first" or resume from a known position — some skew is still possible in principle. Prefer leaving `maxEvents` unbounded, or generous, if you rely on this ordering guarantee.
